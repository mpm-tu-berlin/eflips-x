...
In the bare visualize() method, we try to create undecorated, standard matplotlib (or plotly…) figures. We try to do it in a way that is conducive to a subclass pre- and/or postprocessing our visualize method in order to apply a custom visual style. 

## Accessing databases

Database files should be accessed from the command line using `spatialite` instead of `sqlite3`.

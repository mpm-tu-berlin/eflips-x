...
"""
SWU GTFS flow demonstrating GTFS ingestion and scheduling.
...
from eflips.model import ChargeType, VehicleType, Rotation
...
BATTERY_CAPACITY = 240.0  # kWh - typical for electric buses
...
class ConfigureVehicleTypes(Modifier):
    ...
    The values are hardcoded constants defined at the module level for the SWU use case.
    """
...
@flow
def swu_gtfs_flow() -> None:
    """EFLiPS flow for GTFS ingestion, vehicle scheduling, and simulation for SWU.

    This flow demonstrates a complete electric bus fleet analysis workflow:

    Phase 1 - Data Ingestion and Setup:
        - Ingest GTFS data for SWU (Stadtwerke Ulm) bus network
    ...
    work_dir = Path(gettempdir()) / ("eflips_swu_gtfs_flow")
    ...
    path_to_gtfs_file = (
        path_to_this_file.parent.parent.parent.parent / "data" / "input" / "GTFS" / "SWU.zip"
    )
    ...
    # SWU operates in Ulm, Germany - using approximate coordinates
    depot_config = {
        "depot_station": (9.9677346, 48.396395),  # (lon, lat) - Ulm Betriebshof
        "name": "SWU Depot Ulm",

...
# the same expensive pipeline result without duplicating setup code.
# ---------------------------------------------------------------------------


@pytest.fixture
def simulated_scenario(simulated_db) -> None:
    """Compatibility shim: tests that accept simulated_scenario get None (unused)."""
    return None


@pytest.fixture
def db_session(simulated_session):
    """Override conftest db_session: each test gets a rollback session into the shared simulated DB."""
    return simulated_session


@pytest.fixture
def temp_db(simulated_db_path) -> Path:
    """Override conftest temp_db: points to the shared simulated DB file."""
    return simulated_db_path
...
class TestDepartureArrivalSocAnalyzer:
    ...
    def test_analyze(
        self,
        analyzer: DepartureArrivalSocAnalyzer,
        temp_db: Path,
        simulated_scenario,
        db_session: Session,
    ):
    ...
    def test_visualize(
        ...
        fig.write_html(Path(tempfile.gettempdir()) / "departure_arrival_soc.html")
...
class TestDepotEventAnalyzer:
    ...
    def test_visualize(
        ...
        for color_scheme in ["event_type", "location", "area_type"]:
            ...
            # Write to file for debugging
            fig.write_html(Path(tempfile.gettempdir()) / f"depot_event_{color_scheme}.html")
...
class TestPowerAndOccupancyAnalyzer:
    ...
    def test_visualize(
        ...
        # Write to file for debugging
        fig.write_html(Path(tempfile.gettempdir()) / "power_and_occupancy.html")
...
class TestSpecificEnergyConsumptionAnalyzer:
    ...
    def test_analyze(
        self,
        analyzer: SpecificEnergyConsumptionAnalyzer,
        temp_db: Path,
        simulated_scenario,
        db_session: Session,
    ):
    ...
    def test_visualize(
        ...
        # Write to file for debugging
        fig.write_html(Path(tempfile.gettempdir()) / "specific_energy_consumption.html")
...
class TestVehicleSocAnalyzer:
    ...
    def test_visualize(
        ...
        # Write to file for debugging
        fig.write_html(Path(tempfile.gettempdir()) / "vehicle_soc.html")
...
class TestDepotActivityAnalyzer:
    ...
    def test_visualize(
        ...
        animation_start = (
            db_session.query(Event).order_by(Event.time_start.asc()).first().time_start
        )
        animation_end = db_session.query(Event).order_by(Event.time_end.desc()).first().time_end
        ...
        # Write to file for debugging
        anim.save(Path(tempfile.gettempdir()) / "depot_activity.mp4")

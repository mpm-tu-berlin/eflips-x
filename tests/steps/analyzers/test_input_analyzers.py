...
class TestRotationInfoAnalyzer:
    ...
    def test_visualize_with_timezone(
        ...
        # save map to HTML for manual inspection
        temp_dir = tempfile.gettempdir()
        fig.write_html(Path(temp_dir) / "test_rotation_info_utc.html")
...
class TestGeographicTripPlotAnalyzer:
    ...
    def test_analyze_all_trips(
        self,
        analyzer: GeographicTripPlotAnalyzer,
        temp_db: Path,
        test_scenario: Scenario,
        db_session: Session,
    ):
    ...
    def test_visualize(
        ...
        # save map to HTML for manual inspection
        temp_dir = tempfile.gettempdir()
        map_obj.save(Path(temp_dir) / "test_geographic_trip_plot.html")
...
class TestSingleRotationInfoAnalyzer:
    ...
    def test_visualize(
        ...
        # Plot the cytoscape object to a temporary HTML file for manual inspection
        if False:
            app = Dash()
            app.layout = html.Div(cyto_obj)
            app.run(debug=True)
